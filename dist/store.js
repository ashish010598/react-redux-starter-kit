...
export default function configureStore(globalReducer, initialState = {}) {
  ...
  const store = createStore(createReducer(preLoadedReducers), initialState, composeEnhancers(...enhancers));
  const persistor = persistStore(store); // Extensions
  ...
  store.preLoadedReducers = preLoadedReducers; // sagaMiddleware.run(sagas);
  ...
  if (module.hot) {
    module.hot.accept('./utils/createReducer', () => {
      store.replaceReducer(createReducer(preLoadedReducers, store.injectedReducers));
    });
  }
